...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-mobilenetv3-large-final-full-size-da.h5'
...
# 以訓練好的 MobileNetV3Large 為基礎來建立模型，
# 捨棄 MobileNetV3Large 頂層的 fully connected layers
net = MobileNetV3Large(include_top=False, weights='imagenet', input_tensor=None, 
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-mobilenetv3-large-full-size-da.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-mobilenetv3-large-full-size-da-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
# 顯示模型訓練參數
print("Trainable Parameters：%s" % add_commas(count_params(net_final.trainable_weights)))
...
plt.ylabel('True')
# plt.show()
plt.savefig('confusion_matrix-mobilenetv3-large-full-size-da.png')
end = time.time()
