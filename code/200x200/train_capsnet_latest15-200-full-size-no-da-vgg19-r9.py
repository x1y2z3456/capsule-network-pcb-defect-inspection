...
def spread_loss(y_true, y_pred):
    ...
    # spread loss
    output1 = tf.reshape(y_pred, shape=[40, 1, num_class])
...
# x = layers.Conv2D(512, (9, 9), activation='relu')(base_model.layers[-1].output)
...
lr = 1e-4
adam = Adam(lr=lr)
# reduce_lr = callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.9999, patience=10, min_lr=0.0001)
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-capsnet-latest-15-200-full-size-da-vgg19-r9.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-capsnet-latest-15-200-full-size-da-vgg19-r9-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-capsnet-latest-15-200-full-size-da-vgg19-r9.h5'
# 儲存訓練好的模型
...
plt.ylabel('True')
plt.savefig('cm_capsnet_latest15-200-full-size-da-vgg19-r9.png')
# plt.show()
end = time.time()
