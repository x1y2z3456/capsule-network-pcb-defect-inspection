...
# x = layers.Conv2D(filters=256, kernel_size=9, strides=1, activation='relu')(input_image)
# x = layers.Conv2D(filters=256, kernel_size=9, strides=2)(x)
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-capsnet-origin-2-full-size-100-no-da-final-r2.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-capsnet-origin-2-full-size-100-no-da-final-r2-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-capsnet-origin-2-full-size-100-no-da-final-r2.h5'
# 儲存訓練好的模型
...
plt.ylabel('True')
plt.savefig('confusion_matrix-capsnet-origin-2-full-size-100-no-da-final-r2.png')
# plt.show()
end = time.time()
