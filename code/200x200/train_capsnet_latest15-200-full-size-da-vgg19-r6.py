...
def spread_loss(y_true, y_pred):
    margin = 0.2
    lamb = random.uniform(0.1, 0.91)
    return tf.reduce_sum(tf.square(lamb + margin - tf.keras.backend.relu(y_true - y_pred)), axis=-1)
...
x = layers.Reshape((-1, 512))(base_model.layers[-1].output)
...
# FREEZE_LAYERS = len(model.layers) - 5 - 5
...
adam = Adam(lr=0.001)
model.compile(loss=spread_loss, optimizer=adam, metrics=['accuracy'])
# model.summary()
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-capsnet-latest-15-200-full-size-da-vgg19-r6.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-capsnet-latest-15-200-full-size-da-vgg19-r6-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=True, verbose=1)
...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-capsnet-latest-15-200-full-size-da-vgg19-r6.h5'
# 儲存訓練好的模型
...
plt.ylabel('True')
plt.savefig('cm_capsnet_latest15-200-full-size-da-vgg19-r6.png')
# plt.show()
end = time.time()
