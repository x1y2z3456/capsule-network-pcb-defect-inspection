...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-mobilenetv2-final-full-size-da.h5'
...
# 捨棄 MobileNetV2 頂層的 fully connected layers
net = MobileNetV2(include_top=False, weights='imagenet', input_tensor=None, 
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-mobilenetv2-full-size-da.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-mobilenetv2-full-size-da-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
plt.ylabel('True')
# plt.show()
plt.savefig('confusion_matrix-mobilenetv2-full-size-da.png')
end = time.time()
