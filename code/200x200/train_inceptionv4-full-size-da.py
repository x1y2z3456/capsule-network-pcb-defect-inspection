...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-inceptionv4-final-full-size-da.h5'
...
# 以訓練好的 InceptionV4 為基礎來建立模型，
# 捨棄 InceptionV4 頂層的 fully connected layers
net = inception_v4.create_model(include_top=False, weights='imagenet')
x = net.output
x = Flatten()(x)
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-inceptionv4-full-size-da.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-inceptionv4-full-size-da-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
plt.ylabel('True')
# plt.show()
plt.savefig('confusion_matrix-inceptionv4-full-size-da.png')
end = time.time()
