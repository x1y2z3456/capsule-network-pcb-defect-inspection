...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-efficientb0final-full-size-da-r2.h5'
...
# 以訓練好的 EfficientNetB0 為基礎來建立模型，
# 捨棄 EfficientNetB0 頂層的 fully connected layers
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-efficientb0full-size-da-r2.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-efficientb0full-size-da-r2-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
plt.ylabel('True')
# plt.show()
plt.savefig('confusion_matrix-efficientb0full-size-da-r2.png')
end = time.time()
