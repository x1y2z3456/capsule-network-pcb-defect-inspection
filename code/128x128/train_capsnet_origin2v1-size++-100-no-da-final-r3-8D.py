...
def squash(x, axis=-1):
    ...
    scale = tf.sqrt(s_squared_norm) / (1.0 + s_squared_norm)
    return scale * x
...
# x = layers.Conv2D(filters=128, kernel_size=7, strides=1, activation='relu')(input_image)
# x = layers.Conv2D(filters=128, kernel_size=7, strides=1)(x)
...
# callbacks
log = callbacks.CSVLogger(DATASET_PATH + 'log-capsnet-origin2v1-8D-2-size++-100-no-da-final-r3.csv')
checkpoint = callbacks.ModelCheckpoint(DATASET_PATH + 'weights-capsnet-origin2v1-8D-2-size++-100-no-da-final-r3-{epoch:02d}.h5', monitor='val_acc', save_best_only=True, save_weights_only=False, verbose=1)
...
# 模型輸出儲存的檔案
WEIGHTS_FINAL = 'model-capsnet-origin2v1-8D-2-size++-100-no-da-final-r3.h5'
# 儲存訓練好的模型
...
plt.ylabel('True')
plt.savefig('confusion_matrix-capsnet-origin2v1-8D-2-size++-100-no-da-final-r3.png')
# plt.show()
end = time.time()
