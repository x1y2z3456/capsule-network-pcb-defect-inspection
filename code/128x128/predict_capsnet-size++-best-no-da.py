...
model.load_weights('weights-capsnet-latest-15-200-no-da-96.h5')
...
output = layers.Lambda(lambda z: K.sqrt(K.sum(K.square(z), 2)))(capsule)
model = models.Model(inputs=input_image, outputs=output)
model.load_weights('weights-capsnet-latest-15-200-no-da-164.h5')

# model.summary()

y_pred_org_200 = predict_model(model, test_set)

model.load_weights('weights-capsnet-latest-15-200-no-da-after-no-da-56.h5')
...
model.load_weights('weights-capsnet-origin-2-size+-300-297.h5')
...
# 定義4個模型的預測結果
print('DenseNet-121')
model_1 = models.load_model('model-densenet121-final-size++-no-da.h5')
y_pred_1_org = predict_model(model_1, test_set)

print('ResNet-50')
model_2 = models.load_model('model-ResNet50-final-size++-no-da.h5')
y_pred_2_org = predict_model(model_2, test_set)

print('VGGNet-19')
model_3 = models.load_model('model-VGG19-final-size++-no-da.h5')
