...
def plot_fig_with_text(filename, nrows):
    ...
    accuracy = np.array(data['acc'])
    val_accuracy = np.array(data['val_acc'])
...
#     step = round(np.ceil(nrows / num_ticks))  # 刻度間距
    tick_step = {20: 2, 40: 5, 60: 10, 80: 10, 100: 10, 200: 10, 300: 20}
    

    # 创建网格布局
    fig = plt.figure(figsize=(12, 55))
    gs = gridspec.GridSpec(nrows=4, ncols=1, height_ratios=[1.5, 1.5, 4.5, 1.5], hspace=0.07)

    # 绘制训练准确度图表
    ax1 = fig.add_subplot(gs[0])
    ax1.plot(data['epoch'], data['acc'], label='Accuracy')
    ax1.plot(data['epoch'], data['val_acc'], label='Validation Accuracy')
    ax1.scatter(last_row['epoch'], last_row['acc'], label=f'Accuracy: {last_row["acc"]:.4f}')
    ax1.scatter(last_row['epoch'], last_row['val_acc'], label=f'Val Accuracy: {last_row["val_acc"]:.4f}')
    ...
    # 添加训练准确度的散点标记
    ax1.annotate(f"Max Val Accuracy: {max_row['val_acc']:.4f}\nCurrent Accuracy: {max_row['acc']:.4f}\nCurrent Val Loss: {max_row['val_loss']:.4f}\nCurrent Loss: {max_row['loss']:.4f}\nEpoch: {max_row['epoch']:.0f}", 
                 xy=(max_row['epoch'], max_row['val_acc']), 
...
#     print(valleys)
    # 資料
    table_data = [['epoch', 'loss', 'val loss', 'acc', 'val acc'],]
...
print('New Capsnet V1 100 epoch')
plot_fig_with_text('log-capsnet-new-v1-size-no-da.csv', 100)
print('Origin Capsnet 300 epoch')
plot_fig_with_text('log-capsnet-origin-300-2-no-da-r-size.csv', 300)
print('')
print('DenseNet121 100 epoch')
plot_fig_with_text('log-densenet121-size-no-da.csv', 100)
print('ResNet50 100 epoch')
plot_fig_with_text('log-ResNet50-size-no-da.csv', 100)
print('VGG19 100 epoch')
plot_fig_with_text('log-vgg19-size-no-da.csv', 100)
