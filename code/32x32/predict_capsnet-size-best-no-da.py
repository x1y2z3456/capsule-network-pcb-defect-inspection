...
# x = Capsule(num_classes, 8, 3, True)(x)  
# capsule = Capsule(num_classes, 8, 3, True)(x) 
...
print('New CapsNet')
model.load_weights('weights-capsnet-new-v1-size-no-da-97.h5')
...
model.load_weights('weights-capsnet-origin-300-2-no-da-r-size-285.h5')
...
print('VGGNet-19')
model_3 = models.load_model('model-VGG19-final-size.h5')
y_pred_3_org = predict_model(model_3, test_set)

# 計算每個模型的FPR，TPR和閾值
fpr_0, tpr_0, thresholds_0 = roc_curve(y_true, y_pred_org[:, 1])
...
# 繪製所有模型的ROC曲線
plt.plot(fpr_0, tpr_0, color='blue', lw=2, label='New CapsNet (AUC = %0.4f)' % auc_0)
